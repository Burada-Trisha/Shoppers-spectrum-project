...
# We will use a dictionary to store loading errors for debugging
load_errors = {}
...
def load_models():
    try:
        ...
        import traceback
        load_errors['models'] = traceback.format_exc()
...
def load_data():
    try:
        df = pd.read_csv('cleaned_transactions.csv.gz')
        df['InvoiceDate'] = pd.to_datetime(df['InvoiceDate'])
        rfm = pd.read_csv('customer_rfm.csv')
        return df, rfm
    except Exception as e:
        import traceback
        load_errors['data'] = traceback.format_exc()
        return None, None
...
if kmeans is None or df_transactions is None:
    ...
    if 'models' in load_errors:
        st.write("### Model Load Error Debug Trace:")
        st.code(load_errors['models'])
    if 'data' in load_errors:
        st.write("### Data Load Error Debug Trace:")
        st.code(load_errors['data'])
